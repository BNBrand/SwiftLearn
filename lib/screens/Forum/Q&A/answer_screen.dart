...
class _AnswerScreenState extends State<AnswerScreen> {
  ...
  Future _getAnswers() async{
    await FirebaseFirestore.instance.collection('questions').doc(widget.questionId).collection('answers')
        .get().then((snapshots){
      if(snapshots.docs.isNotEmpty){
        FirebaseFirestore.instance.collection('questions').doc(widget.questionId).update(
            {'answers': snapshots.docs.length });
      }else{
        FirebaseFirestore.instance.collection('questions').doc(widget.questionId).update(
            {'answers': 0});
      }
